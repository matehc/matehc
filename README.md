# Lucas Okafor
[![AltSchool Badge](https://img.shields.io/badge/-Engineering-6773E5?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&link=https://altschoolafrica.com/schools/engineering)](https://altschoolafrica.com/schools/engineering)


[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue)]([www.linkedin.com/in/lucas-okafor](https://www.linkedin.com/in/lucas-okafor-29227a13a/))
[![GitHub](https://img.shields.io/badge/GitHub-Follow-lightgrey)](www.github.com/matehc)

## About Me
I am an IT Technical Support Specialist aspiring to become a Frontend Web Developer. I possess a solid understanding of HTML, CSS, JavaScript, and REACT library.

**Here's a quick summary about me**:

- 😊 Pronouns: He/him
- 🔭 I'm currently working on [Invoice App](https://www.frontendmentor.io/challenges/invoice-app-i7KaLTQjl).
- 🌱 I'm currently learning advanced React and Tailwind.
- 🤔 I'm looking for help with best industry practices for CSS, JavaScript and React.
- 💡 Fun fact: I'm currently studying at AltSchool Africa [School of Software Engineering](https://altschoolafrica.com/schools/engineering) Class of 2022
- 💼 Job interests: Software Engineer, Frontend (Intern or Junior level).
- 📫 How to reach me by emailing: lucas.okafor.career@gmail.com

## Projects

- **Portfolio Site**: Created a Portfolio site from a Figma file using HTML, CSS, and JavaScript. [Link](https://matehc.github.io/portfolio__frontendmentor/).
- **RedactR Application**: Led a team of 5 in a hackathon to develop a RedactR application with HTML, CSS, and JavaScript. The application can redact keywords and received over 1000 views from frontend engineering students. [Link](https://matehc.github.io/redactr-sandbox/).
- **Advice Application**: Implemented an application that consumes data from an API using JavaScript. [Link](https://matehc.github.io/advice-generator_frontend_mentor/).
- **Dictionary App**: Built a Dictionary App from a Figma file using HTML, CSS, and JavaScript. [Link](https://matehc.github.io/dictionary-web-app__frontendmentor/)


##Languages

<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/html/html.png"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/css/css.png"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/javascript/javascript.png"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/typescript/typescript.png"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/react/react.png"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/git/git.png"></code>

Feel free to explore my repositories and projects above to get a closer look at my work. I am open to collaborations and new opportunities, so feel free to reach out to me via LinkedIn or email.

Let's connect and create amazing things together!
